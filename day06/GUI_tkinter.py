...
# 버튼, 이미지, 텍스트 박스 등 
# 시각적인 요소를 통해 프로그램과 상호작용하는 것을 말한다.
# 파이썬에는 기본 내장 라이브러리로
...
class GUI_practice():
  ...
  def settings(self):
    ...
    self.label1 = tk.Label(self.root, text="첫번째 라벨")
    ...
    # 버튼 위젯 생성
    # command 매개변수는 함수를 전달하는 매개변수로(callback)
    # 소괄호 없이 함수 혹은 메서드의 이름만 전달해야 한다.
    self.submit = tk.Button(self.root, text="전송", command=self.btn_click)
  ...
  def btn_click(self):
    ...
    print(self.data)
    # -> self.data에 담는다.
    # self.label1에 그 값을 전달한다.
    self.label1.config(text=self.data)
